...
wss.on('connection', (clientWs, req) => {
  ...
  try {
    ...
    if (!stream) {
      ...
      // ИСПРАВЛЕНО: Читаем данные напрямую из объекта события SDK, а не из msg.data
      liveClient.on('like', (event) => {
        // Добавил лог для отладки, чтобы вы видели лайки в консоли Railway
        console.log(`[LIKE] ${username} <- ${event.user?.uniqueId} (${event.likeCount} likes)`);
      ...
      liveClient.on('gift', (event) => {
        console.log(`[GIFT] ${username} <- ${event.user?.uniqueId} sent ${event.giftName} x${event.repeatCount || 1}`);
...
server.listen(port, '0.0.0.0', () => {
  console.log(`🚀 TikTools Server running on port ${port}`);
});
